...
Main.main = function() {
	...
	var wss = new (require('ws').Server)({ port : 3000});
	wss.on(nodejs.ws.WebSocketServerEventType.Connection,function(p_ws) {
		p_ws.on(nodejs.ws.WebSocketEventType.Message,function(p_data) {
			console.log("WS> " + Std.string(p_data));
		});
