...
Main.__name__ = true;
Main.main = function() {
	var gm = new (require('gm').subClass( { imageMagick: true } ))("img.jpg");
	nodejs.haxe.io.Directory.MkdirRecursive("");
};
var Std = function() { };
Std.__name__ = true;
...
nodejs.haxe = {};
nodejs.haxe.io = {};
nodejs.haxe.io.Directory = function() { };
nodejs.haxe.io.Directory.__name__ = true;
nodejs.haxe.io.Directory.Traverse = function(p_path,p_callback) {
	var p = StringTools.trim(p_path);
	if(p.charAt(p.length - 1) == "/") p = HxOverrides.substr(p,0,p.length - 1);
	nodejs.haxe.io.Directory.TraverseStep(p,"",p_callback);
};
nodejs.haxe.io.Directory.MkdirRecursive = function(p_path,p_mode) {
	if(p_mode == null) p_mode = "0777";
	...
	while(p.length > 0) {
		...
		if(step == "") step += it; else step += "/" + it;
		if(!(require('fs')).existsSync(step)) (require('fs')).mkdirSync(step,p_mode);
	}
};
nodejs.haxe.io.Directory.TraverseStep = function(p_root,p_current,p_callback) {
	var p = p_root + p_current;
	if(p.indexOf(".") >= 0) {
		p_callback(p);
		return;
	}
	var list = (require('fs')).readdirSync(p);
	var _g1 = 0;
	var _g = list.length;
	while(_g1 < _g) {
		var i = _g1++;
		nodejs.haxe.io.Directory.TraverseStep(p + "/",list[i],p_callback);
	}
};
nodejs.http = {};
